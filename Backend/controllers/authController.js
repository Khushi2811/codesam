...
const sendVerificationEmail = async (email, otp) => {
  try {
    ...
    const mailOptions = {
      from: `"Code Sam" <${process.env.EMAIL_USER}>`, // Add a proper from name
      to: email,
...
const resendOtp = async (req, res) => {
  try {
      ...
      // Send verification email with new OTP using sendResendOtpEmail
      try {
          await sendResendOtpEmail(user.email, otp); // Use new function here
      ...
      return res.status(500).json({ error: 'An error occurred while resending OTP', details: error.message });
  }
};
...
const sendResendOtpEmail = async (email, otp) => {
  try {
      ...
      const mailOptions = {
          from: `"Code Sam" <${process.env.EMAIL_USER}>`, // Add a proper from name
          to: email,
          subject: 'Resend: Email Verification for Your Account',
          html: `
              <h1>Email Verification</h1>
              <p>You requested to resend the OTP. Please use the following OTP to verify your email address:</p>
              <h2 style="color: #4CAF50; letter-spacing: 2px;">${otp}</h2>
              <p>This OTP will expire in 1 minute.</p>
              <p>If you didn't request this verification, please ignore this email.</p>
          `
      ...
      console.error('Detailed resend email error:', error);
      throw error;
  }
};
